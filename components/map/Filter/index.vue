<template>
  ...
  >
    ...
    >
      ...
      <!-- Conteúdo -->
      <div class="py-4 mx-8 px-1 relative z-10 w-full" ref="filterContentRef">
        <!-- Filtro -->
        <div class="flex pt-2 pb-4 items-center justify-between">
          ...
          <button
            type="button"
            @click="limparFiltros"
            class="text-yellow-300 hover:text-yellow-300/90 cursor-pointer relative transition-colors duration-200 border-none bg-transparent p-0"
        ...
        >
          ...
          >
            ...
            <div class="flex flex-col gap-4">
              ...
              <div>
                <div
                  class="self-stretch flex justify-between items-center gap-2"
                >
                  <!-- Icon -->
                  <SvgoCar
                  ...
                  <!-- Toogle -->
                  <UiToggle
                    v-model="filtros.acaoPolicial"
                    aria-label="Filtrar apenas vítimas em ocorrências que houveram operações ou ações policiais"
                  />
                </div>
              ...
              <div>
                ...
                >
                  ...
                  <!-- Toogle -->
                  <UiToggle
                    v-model="filtros.balaPerdida"
                    aria-label="Filtrar apenas vítimas em circunstância de bala perdida"
                  />
                </div>
                <div class="text-blue-300 text-sm font-light leading-tight">
              ...
              <div>
                ...
                >
                  ...
                  <!-- Toogle -->
                  <UiToggle
                    v-model="filtros.dentroDeCasa"
                    aria-label="Filtrar apenas vítimas baleadas em uma residência"
                  />
                </div>
                <div class="text-blue-300 text-sm font-light leading-tight">
...
<script setup>
import { inject, defineProps, defineEmits, ref, watch, nextTick } from 'vue';
...
// Quando o filtro é aberto, move o foco para o primeiro elemento focável
watch(
  () => props.showAside,
  (isOpen) => {
    if (isOpen) {
      nextTick(() => {
        // Procura o primeiro elemento focável dentro do filtro (botão "Limpar" ou primeiro toggle)
        const firstFocusable = filterContentRef.value?.querySelector(
